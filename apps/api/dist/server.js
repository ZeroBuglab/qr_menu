...
const error = (reply, statusCode, code, message) => reply.code(statusCode).send({ success: false, error: { code, message } });
const slugSchema = z.object({ slug: z.string().min(2).max(80).regex(/^[a-z0-9-]+$/) });
const sessionName = "qr_session";
const hashToken = (token) => createHash("sha256").update(token).digest("hex");
const getSession = async (request) => { const token = request.cookies[sessionName]; if (!token)
    return null; return prisma.session.findUnique({ where: { tokenHash: hashToken(token) }, include: { user: { include: { memberships: { include: { role: true, restaurant: true } } } } } }); };
const requireRole = async (request, reply, allowed) => { const session = await getSession(request); const membership = session?.user.memberships.find((entry) => allowed.includes(entry.role.name)); if (!session || session.expiresAt < new Date() || !membership) {
    error(reply, 401, "UNAUTHORIZED", "Authentication required");
    return null;
...
app.post("/api/v1/auth/login", { schema: { tags: ["auth"] } }, async (request, reply) => { const parsed = loginSchema.safeParse(request.body); if (!parsed.success)
    return error(reply, 422, "VALIDATION_ERROR", "Invalid credentials"); const user = await prisma.user.findUnique({ where: { email: parsed.data.email.toLowerCase() }, include: { memberships: { include: { role: true, restaurant: true } } } }); if (!user || !user.isActive || !(await argon2.verify(user.passwordHash, parsed.data.password)))
    return error(reply, 401, "INVALID_CREDENTIALS", "Invalid credentials"); const raw = randomBytes(32).toString("base64url"); await prisma.session.create({ data: { userId: user.id, tokenHash: hashToken(raw), expiresAt: new Date(Date.now() + 1000 * 60 * 60 * 24 * 14) } }); reply.setCookie(sessionName, raw, { httpOnly: true, secure: process.env.NODE_ENV === "production", sameSite: "lax", path: "/", maxAge: 60 * 60 * 24 * 14 }); return { success: true, data: { id: user.id, displayName: user.displayName, memberships: user.memberships.map(({ role, restaurant }) => ({ role: role.name, restaurantId: restaurant.id, restaurant: restaurant.name })) } }; });
app.post("/api/v1/auth/logout", { schema: { tags: ["auth"] } }, async (request, reply) => { const token = request.cookies[sessionName]; if (token)
    await prisma.session.deleteMany({ where: { tokenHash: hashToken(token) } }); reply.clearCookie(sessionName, { path: "/" }); return { success: true, data: null }; });
app.get("/api/v1/auth/me", { schema: { tags: ["auth"] } }, async (request, reply) => { const current = await getSession(request); if (!current || current.expiresAt < new Date())
    return error(reply, 401, "UNAUTHORIZED", "Authentication required"); return { success: true, data: { id: current.user.id, displayName: current.user.displayName, memberships: current.user.memberships.map(({ role, restaurant }) => ({ role: role.name, restaurantId: restaurant.id, restaurant: restaurant.name })) } }; });
app.get("/api/v1/restaurants/:slug/menu", { schema: { tags: ["menu"] } }, async (request, reply) => {
    ...
    if (!parsed.success)
        return error(reply, 400, "VALIDATION_ERROR", "Invalid restaurant slug");
    ...
    if (!restaurant || restaurant.status !== "ACTIVE")
        return error(reply, 404, "RESTAURANT_NOT_FOUND", "Restaurant not found");
...
app.get("/api/v1/orders/track/:token", async (request, reply) => {
    ...
    if (!/^(?:[A-Za-z0-9_-]{20,100}|KOR-\d{4})$/.test(token))
        return error(reply, 404, "ORDER_NOT_FOUND", "Order not found");
...
app.post("/api/v1/orders", { schema: { tags: ["orders"] } }, async (request, reply) => {
    ...
    if (!parsed.success)
        return error(reply, 422, "VALIDATION_ERROR", "Invalid order payload");
    const input = parsed.data;
    const restaurant = await prisma.restaurant.findUnique({ where: { id: input.restaurantId }, include: { settings: true } });
    if (!restaurant || restaurant.status !== "ACTIVE" || !restaurant.settings)
        return error(reply, 404, "RESTAURANT_NOT_FOUND", "Restaurant not found");
    let resolvedTableId = input.tableId;
    if (input.tableNumber) {
        const table = await prisma.restaurantTable.findFirst({ where: { number: input.tableNumber, restaurantId: input.restaurantId, isActive: true } });
        if (!table)
            return error(reply, 422, "INVALID_TABLE", "Table is not available");
        resolvedTableId = table.id;
    }
    if (resolvedTableId) {
        const table = await prisma.restaurantTable.findFirst({ where: { id: resolvedTableId, restaurantId: input.restaurantId, isActive: true } });
        if (!table)
            return error(reply, 422, "INVALID_TABLE", "Table is not available");
    }
    ...
    if (products.length !== new Set(input.items.map((item) => item.productId)).size)
        return error(reply, 422, "ITEM_UNAVAILABLE", "One or more items are unavailable");
    const productById = new Map(products.map((product) => [product.id, product]));
    for (const item of input.items) {
        const product = productById.get(item.productId);
        if (item.variantId && !product.variants.some((variant) => variant.id === item.variantId))
            return error(reply, 422, "INVALID_VARIANT", "One or more variants are invalid");
        if (item.modifierIds.some((id) => !product.modifiers.some((modifier) => modifier.modifierId === id)))
            return error(reply, 422, "INVALID_MODIFIER", "One or more modifiers are invalid");
    }
    ...
    if (total < restaurant.settings.minimumOrderAmount)
        return error(reply, 422, "MINIMUM_ORDER", "Order total is below the minimum");
...
app.patch("/api/v1/orders/:id/status", { schema: { tags: ["orders"] } }, async (request, reply) => { const auth = await requireRole(request, reply, ["OWNER", "MANAGER", "KITCHEN", "WAITER"]); if (!auth)
    return; const body = statusUpdate.safeParse(request.body); if (!body.success)
    return error(reply, 422, "VALIDATION_ERROR", "Invalid status"); const role = auth.membership.role.name; if (role === "KITCHEN" && !["COOKING", "READY", "CANCELLED"].includes(body.data.status))
    return error(reply, 403, "FORBIDDEN", "Kitchen role cannot perform this transition"); if (role === "WAITER" && !["DELIVERING", "COMPLETED", "CANCELLED"].includes(body.data.status))
    return error(reply, 403, "FORBIDDEN", "Waiter role cannot perform this transition"); const order = await prisma.order.findUnique({ where: { id: String(request.params.id) } }); if (!order || order.restaurantId !== auth.membership.restaurantId)
    return error(reply, 404, "ORDER_NOT_FOUND", "Order not found"); if (!canTransition(order.status, body.data.status))
    return error(reply, 409, "INVALID_STATE_TRANSITION", "Order status cannot be changed that way"); const updated = await prisma.$transaction(async (tx) => { const next = await tx.order.update({ where: { id: order.id }, data: { status: body.data.status, estimatedMinutes: body.data.estimatedMinutes } }); await tx.orderStatusHistory.create({ data: { orderId: order.id, fromStatus: order.status, toStatus: body.data.status } }); await tx.auditLog.create({ data: { restaurantId: order.restaurantId, actorId: auth.session.user.id, action: "ORDER_STATUS_CHANGED", entity: "Order", entityId: order.id, metadata: { from: order.status, to: body.data.status } } }); return next; }); socketServer?.to(`order:${order.id}`).emit(`order.${body.data.status.toLowerCase()}`, updated); return { success: true, data: updated }; });
socketServer = new SocketServer(app.server, { cors: { origin: env.origin, credentials: true } });
socketServer.on("connection", (socket) => { socket.on("order.subscribe", async (payload) => { if (!payload || typeof payload.orderId !== "string" || typeof payload.trackingToken !== "string")
    return; const order = await prisma.order.findFirst({ where: { id: payload.orderId, trackingToken: payload.trackingToken }, select: { id: true } }); if (order)
